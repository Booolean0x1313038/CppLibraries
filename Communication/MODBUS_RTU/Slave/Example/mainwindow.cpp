...
#pragma region { // 初始化寄存器
    ...
    _RegistersAttribute_Device[6] = MODBUS_RTU_REGISTER_ATTRIBUTE_READABLE;
    for (int i = 7; i < MODBUS_REGISTERS_DEVICE_AMOUNT; i++)
    {
        _RegistersAttribute_Device[i] = (MODBUS_RTU_Register_Attribute)((unsigned char)MODBUS_RTU_REGISTER_ATTRIBUTE_READABLE | (unsigned char)MODBUS_RTU_REGISTER_ATTRIBUTE_WRITABLE);
    }
    _CoiledRegistersArray[0].Registers = _Registers_Device;
    _CoiledRegistersArray[0].Bytes = sizeof(_Registers_Device);
    _CoiledRegistersArray[0].RegistersAttribute = _RegistersAttribute_Device;
    _CoiledRegistersArray[0].FirstRegisterAddress = MODBUS_REGISTERS_DEVICE_FIRSTADDRESS;
#ifdef USE_COIL
    _CoiledRegistersArray[0].RegisterType = MODBUS_RTU_REGISTER_TYPE_UINT16;
#endif
    _CoiledRegistersArray[0].WriteTimes = 2;
    for (int i = 0; i < _CoiledRegistersArray[0].Bytes; i++)
    {
        ((uint8_t*)_CoiledRegistersArray[0].Registers)[i] = i;
    }
    ...
    {
        _RegistersAttribute_Gradient[i] = (MODBUS_RTU_Register_Attribute)((unsigned char)MODBUS_RTU_REGISTER_ATTRIBUTE_READABLE | (unsigned char)MODBUS_RTU_REGISTER_ATTRIBUTE_WRITABLE);
    }
    _CoiledRegistersArray[1].Registers = _Registers_Gradient;
    _CoiledRegistersArray[1].Bytes = sizeof(_Registers_Gradient);
    _CoiledRegistersArray[1].RegistersAttribute = _RegistersAttribute_Gradient;
    _CoiledRegistersArray[1].FirstRegisterAddress = MODBUS_REGISTERS_GRADIENT_FIRSTADDRESS;
#ifdef USE_COIL
    _CoiledRegistersArray[1].RegisterType = MODBUS_RTU_REGISTER_TYPE_BIT;
#endif
    _CoiledRegistersArray[1].WriteTimes = 1;
    for (int i = 0; i < _CoiledRegistersArray[1].Bytes; i++)
    {
        ((uint8_t*)_CoiledRegistersArray[1].Registers)[i] = 0xFF;//i;
    }
    ...
    {
        _RegistersAttribute_DirectColor[i] = (MODBUS_RTU_Register_Attribute)((unsigned char)MODBUS_RTU_REGISTER_ATTRIBUTE_READABLE | (unsigned char)MODBUS_RTU_REGISTER_ATTRIBUTE_WRITABLE);
    }
    _CoiledRegistersArray[2].Registers = _Registers_DirectColor;
    _CoiledRegistersArray[2].Bytes = sizeof(_Registers_DirectColor);
    _CoiledRegistersArray[2].RegistersAttribute = _RegistersAttribute_DirectColor;
    _CoiledRegistersArray[2].FirstRegisterAddress = MODBUS_REGISTERS_DIRECTCOLOR_FIRSTADDRESS;
#ifdef USE_COIL
    _CoiledRegistersArray[2].RegisterType = MODBUS_RTU_REGISTER_TYPE_UINT16;
#endif
    _CoiledRegistersArray[2].WriteTimes = 1;
...
void MainWindow::SLOT_PushButton_Prase_Clicked()
{
    ...
    if (
        ...
        _MODBUS_RTU_Slave->CheckHeader(
#else
        MODBUS_RTU_Slave_CheckHeader(_MODBUS_RTU_Slave,
#endif
            (unsigned char*)_Frame.constData(), _Frame.length()))
    {
                ...
                (unsigned char*)_Frame.constData(), _Frame.length()))
        {
            ...
            for (int i = 0; i < MODBUS_COILEDREGISTERS; ++i)
            {
                if (_MODBUS_RTU_Slave->_Prase_Calculate_TargetCoiledRegistersAreaPtr == &_CoiledRegistersArray[i])
                {
        ...
        else
        {
            ...
            ui->tableWidget_Result->setItem(startRow + 1, 1, new QTableWidgetItem("0x" + QString("%1 %2").arg(
                                                                                                             ...
                                                                                                             , 4, 16, '0').arg(
#ifdef USE_CPLUSPLUS_CLASS
                                                                                                 MODBUS_RTU_Slave::GetLastResultString
#else
                                                                                                 MODBUS_RTU_Slave_GetLastResultString
#endif
                                                                                                 (_MODBUS_RTU_Slave)).toUpper()));
...
void MainWindow::SLOT_UpdateFrame()
{
    ...
    uint16_t crc =
        ...
        MODBUS_RTU_Slave::
#endif
        CalculateCRC((unsigned char*)_Frame.constData(), _Frame.length());
    _Frame.append(crc);
    _Frame.append(crc >> 8);
    ui->lineEdit_CRC->setText(QString("%1 %2").arg(crc & 0xFF, 2, 16, '0').arg(crc >> 8, 2, 16, '0').toUpper());
    QString str = "";
...
void MainWindow::ShowRegisterArea(int index)
{
    ...
    MODBUS_RTU_CoiledRegistersArea* area = _CoiledRegistersArray + index;
